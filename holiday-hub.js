...
(function(){const wrap=document.querySelectorAll('.activity');wrap.forEach(card=>{const k=card.dataset.activity;const up=card.querySelector('.up'),down=card.querySelector('.down');const s=JSON.parse(localStorage.getItem('vote_'+k)||'{"up":0,"down":0,"choice":""}');function show(){up.querySelector('span').textContent=s.up;down.querySelector('span').textContent=s.down;up.classList.toggle('active',s.choice==='up');down.classList.toggle('active',s.choice==='down');}function vote(v){if(s.choice===v){s[v]--;s.choice='';}else{if(s.choice==='up')s.up--;if(s.choice==='down')s.down--;s[v]++;s.choice=v;}localStorage.setItem('vote_'+k,JSON.stringify(s));show();}up.addEventListener('click',()=>vote('up'));down.addEventListener('click',()=>vote('down'));show();});})();
